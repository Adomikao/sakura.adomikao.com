...
class MY_Model extends CI_Model
{
     ...
     * @return int 符合数量
     */
    public function get_count_all($where = false, $table = false, $like = '')
    {
        if (!$table) {
            $table = $this->table;
        }
     ...
     * 为普通index提供分页的list列表
     */
    public function get_list($limit = 15, $start = 0, $order = false, $where = false, $fields = '*', $table = false, $like = '')
    {
        if (!$table) {
            $table = $this->table;
        }
    ...
    public function find_one($value, $field = "id", $table = false)
    {
        ...
        $query = $this->db->select('id')->from($table)->where($field, $value)->get();
     ...
     * @return int           影响数
     */
    public function del($ids, $where = [], $table = '')
    {
        if (empty($table)) {
            $table = $this->table;
        }
    ...
    // 组合简单处理 ，建议不要继承了单独做个吧
    public function _where($where, $table = '')
    {
        if (empty($table)) {
            $table = $this->table;
        }
    ...
    public function role($flag = 1, $ids, $where = false, $table = false)
    {
        if (!$table) {
            $table = $this->table;
        }
        if (!$this->db->field_exists('role', $table)) {
            return false;
        }
        if (is_numeric($ids)) {
            $this->db->where(array('id' => $ids));
        ...
        $data = array('role' => $flag);
        $this->db->update($table, $data);
        return $this->db->affected_rows();
    }

    public function ai($hard = 1, $ids, $where = false, $table = false)
    {
        if (!$table) {
            $table = $this->table;
        }
        if (!$this->db->field_exists('aiauth', $table)) {
            return false;
        }
        if (is_numeric($ids)) {
            $this->db->where(array('id' => $ids));
        ...
        $data = array('aiauth' => $hard);
        $this->db->update($table, $data);
        return $this->db->affected_rows();
    }

    public function flag2($hard = 1, $ids, $where = false, $table = false)
    {
        if (!$table) {
            $table = $this->table;
        }
        if (!$this->db->field_exists('flag2', $table)) {
            return false;
        }
        if (is_numeric($ids)) {
            $this->db->where(array('id' => $ids));
        ...
        $data = array('flag2' => $hard);
        $this->db->update($table, $data);
        return $this->db->affected_rows();
    }

    public function comment($hard = 1, $ids, $where = false, $table = false)
    {
        if (!$table) {
            $table = $this->table;
        }
        if (!$this->db->field_exists('commentauth', $table)) {
            return false;
        }
        if (is_numeric($ids)) {
            $this->db->where(array('id' => $ids));
