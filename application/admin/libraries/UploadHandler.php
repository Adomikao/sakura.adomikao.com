...
// TODO: 上传对 accept_file_types 限制时 swf 命名后缀问题。
...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|doc?)$/i',
    ...
    protected function get_user_path() {
        if ($this->options['user_dirs'] === true) {
            return $this->get_user_id().'/';
        }elseif($this->options['user_dirs'] and $this->options['user_dirs'] !== true){
    ...
    protected function random_file_name($name, $type, $index, $content_range) {
       ...
       // Add missing file extension for known image types:
       $ext = '';
       if (preg_match($this->options['accept_file_types'], $name,$matches)) {
           $ext = '.'.$matches[1];
       }
    ...
    public function delete($print_response = true) {
        ...
        $this->db_delete($file_name);
        return $this->generate_response($response, $print_response);
    }

    // 插入数据库
    protected function db_isnert($files){
        $this->CI->load->model('upload_model','upload');
        //rawurldecode
        foreach ($files as $k => $f) {
            // var_dump($f);
            if (isset($f->error) and $f->error != '') {
                break;
            }
            $d = array();
            $d['name'] = $f->name;
            ...
            $d['type'] = $f->type;
            $d['url'] = $f->url;
            $d['thumb'] = $f->thumbnailUrl;
            $d['delete_url'] = '?file='.rawurlencode($f->name).'&dt='.date('Y/m/d', time());
            $d['timeline'] = time();
            $files[$k]->id = $this->CI->upload->create($d);
            $files[$k]->thumb = $d['thumb'];
         }
         return $files;
    }

    // 删除数据
    protected function db_delete($file_name){
        $url = $this->get_user_path().rawurlencode($file_name);
        $this->CI->load->model('upload_model','upload');
        $where = array('url'=>$url);
        $this->CI->upload->del($where);
    }

    // 水印
    protected function watermark($files){

        if (preg_match("configs",$_SERVER['HTTP_REFERER'])) {
            return false;
        }

        $w = $this->CI->mcfg->get('upload','watermark');
        if ($w) {
            ...
            foreach ($files as $k => $f) {
                if (preg_match('/(gif|png|jpg|jpeg)$/i', $f->url) ) {
                    $config['source_image'] =  UPLOAD_PATH.$f->url;
    ...
    // 压缩
  // protected function scale($files){
      // $this->CI->load->library('image_lib');
