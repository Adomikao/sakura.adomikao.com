...
<div class="boxed">
    <h3><i class="fa fa-pencil"></i> <span class="badge badge-success pull-right"><?php echo $title; ?></span> <?php echo lang('edit') ?></h3>
    ...
    <div class="boxed-inner seamless">
        ...
        <div class="control-group">
            <label class="control-label" for="intro_left">描述—左</label>
            <div class="controls">
                <textarea rows="8" name="intro_left"><?php echo str_replace("<br>", "\n", $it['intro_left']); ?></textarea>
            </div>
        </div>

        <div class="control-group">
            <label class="control-label" for="intro_right">描述—右</label>
            <div class="controls">
                <textarea rows="8" name="intro_right"><?php echo str_replace("<br>", "\n", $it['intro_right']); ?></textarea>
            </div>
        </div>
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <input type="text" id="music" name="music" value="<?php echo $it['music'] ?>">
            </div>
        </div>
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <div class="btn-group">
                    ...
                    <input type="hidden" name="image" class="form-upload" data-more="1" value="<?php echo $it['image'] ?>">
                </div>
            </div>
        </div>
        ...
        <?php if(isset($text)): ?>
            <?php foreach ($text as $value): ?>
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="text" name="page[]" value="<?php echo $value['page'] ?>" placeholder="输入页数（大于1）">
                    ...
                    <div class="controls">
                        <textarea rows="12" name="text[]" placeholder="输入内容"><?php echo $value['content'] ?></textarea>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
...
<script>
    require(['jquery', 'adminer/js/ui', 'adminer/js/media'], function ($, ui, media) {
        media.init();
        var photo = <?php echo json_encode(one_upload($it['photo'])) ?>;
        var image = <?php echo json_encode(list_upload($it['image'])) ?>;
        media.show(photo, 'photo');
        media.show(image, 'image');
        media.sort('photo');
        media.sort('image');
