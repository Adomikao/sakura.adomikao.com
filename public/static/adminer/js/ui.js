...
function($,tools){
	...
	var ui = {
		...
		btn_ai:function(url){
			$(".select-list").delegate('.btn-ajax-ai', 'click', function(event) {
				event.preventDefault();
				var ai = {
					id:$(this).attr('data-id'),
				...
				$.ajax({
					...
					.done(function(data) {
						...
						if (data.status == 1 ) {
							...
							if (ai.ai == 1) {
								msg.type = 'success';
								ai.it.addClass('btn-primary').attr({'data-ai':'0','title':'关闭'});
							}else{
								ai.type = 'notice';
								ai.it.removeClass('btn-primary').attr({'data-ai':'1','title':'开启'});
							}
						}else{
							msg.title = "错误！";
							msg.type = 'error';
		...
		btn_commentauth:function(url){
			$(".select-list").delegate('.btn-ajax-comment', 'click', function(event) {
				event.preventDefault();
				var ai = {
					id:$(this).attr('data-id'),
					ai:$(this).attr('data-commentauth'),
					it:$(this),
					i:$(this).children('i')
				};
				$.ajax({
					...
					.done(function(data) {
						...
						if (data.status == 1 ) {
							...
							if (ai.ai == 1) {
								msg.type = 'success';
								ai.it.addClass('btn-primary').attr({'data-commentauth':'0','title':'关闭'});
							}else{
								ai.type = 'notice';
								ai.it.removeClass('btn-primary').attr({'data-commentauth':'1','title':'开启'});
							}
						}else{
							msg.title = "错误！";
							msg.type = 'error';
		...
		btn_flag2:function(url){
			$(".select-list").delegate('.btn-ajax-flag2', 'click', function(event) {
				event.preventDefault();
				var flag = {
					id:$(this).attr('data-id'),
				...
				$.ajax({
					...
					.done(function(data) {
						...
						if (data.status == 1 ) {
							...
							if (flag.flag2 == 1) {
								msg.type = 'success';
								flag.it.addClass('btn-primary').attr({'data-flag2':'0','title':'取消推荐'});
							}else{
								msg.type = 'notice';
								flag.it.removeClass('btn-primary').attr({'data-flag2':'1','title':'推荐'});
							}
						}else{
							msg.title = "错误！";
							msg.type = 'error';
